...
func InitWorerList(scheduler *scheduler) {
	...
	node2 := NewWorkerTaskSpeec(scheduler, "worker-node-1", 0, 2, 1, 1,
		1, 2, 2)

	node3 := NewWorkerTaskSpeec(scheduler, "worker-node-2", 1, 2, 1, 1,
		1, 2, 2)

	node4 := NewWorkerTaskSpeec(scheduler, "worker-node-3", 0, 2, 1, 1,
		1, 2, 2)
	...
	lotusSealingWorkers.WorkerList[node4.Hostname] = node4
